...
| `oneclick.py` | 전수조사 + 안전조치 + 기록수집 + 분석을 한 번에 |
...
| `test_coin_scanner.py` | ADX 계산 범위·ADX 필터·**철지난 흐름 차단**·장부 대조 |
...
| 얻는 것 | 설명 |
|---|---|
| 손실 상한 고정 | 봇 버그·키 유출·폭주 루프가 나도 서브계좌 잔고까지만 날아간다 |
| 장부 정확 | 서브계좌 잔고 = 봇 손익. 체결가 추정 오차 문제가 사라진다 |
| 봇 충돌 불가 | 봇마다 서브계좌를 나누면 같은 포지션을 두고 싸울 수 없다 |

**API 키 설정 시 반드시:**

- **출금 권한 끄기** — 키가 유출돼도 돈을 빼갈 수 없다. 매매만 되면 충분하다
- **IP 화이트리스트** — VPS 로 옮겼다면 그 서버 IP 만 허용
- 선물 거래 권한만 켜기 (현물·마진 불필요)

서브계좌를 쓰면 `CAPITAL_AUTO_SYNC = True` 로 켜라. 기동할 때마다 거래소
잔고에 자본을 맞춘다 — `TOTAL_CAPITAL` 을 손으로 고칠 일이 없어진다.
(과거 이력: `$100 → $215 → $315 → $509 → $518 → $234` 를 매번 수동 수정)

조정은 `total_pnl` 을 건드리지 않고 `total_capital` 만 옮겨
`total_capital + total_pnl == 거래소 순자산` 이 되게 하므로 승패 기록과
누적손익의 의미가 보존된다. 잔고 조회에 실패하거나 0 이 오면 설정값을 지킨다.

> ⚠ **본계좌에서는 켜지 마라.** 봇 외의 입출금이 섞이면 입금을 "수익" 으로,
> 출금을 "손실" 로 오인한다. 그래서 기본값이 `False` 다.
...
```bash
python oneclick.py "C:\...\영상자동화" --dry-run   # 무엇을 할지만 확인
python oneclick.py "C:\...\영상자동화"             # 실행
```

1. 봇 폴더 전수 조사 (실거래·실행 여부·승률·누적손익)
2. **안전**: 실거래 봇 하나만 남기고 나머지 `LIVE_TRADING=False`
   (원본은 `config.py.bak_날짜` 로 백업)
3. 모든 봇의 과거 기록을 `trades.jsonl` 로 수집
4. 손실 분석 → `report.txt` 저장

**일부러 하지 않는 것**: 돌고 있는 봇을 끄거나 재시작하지 않고, 실거래 봇에
코드를 병합하지 않으며, 포지션을 청산하지 않는다. 워치독이 재시작하면
지켜보지 않는 사이에 새 로직이 적용되므로 그건 사람이 보고 있을 때 해야 한다.

## 철지난 흐름 차단 (coin_scanner)

매매 방향(`trend`)은 **20일 일봉 MA 기울기**로 정하는데, 실제 보유 시간은
1~2시간이다. 20일 추세가 이미 꺾였는데도 그 라벨로 진입하면 지나간 흐름에
올라타는 셈이다. 기존 코드에는 이 구멍이 있었다.

- **4h 역행**: 점수만 `0.4`배 깎고 **통과시켰다** → 이제 차단 (`REQUIRE_4H_ALIGN`)
- **1h 역행**: 차단했지만 `SIDEWAYS` 면 통과 (유지)
- **최근 실제 이동**: 새로 추가. 최근 15m 캔들 6개(1.5시간)의 **실제 가격 이동**이
  방향과 반대로 `RECENT_MOVE_MAX_ADVERSE`(0.3%) 이상 갔으면 차단.
  MA 기울기가 아니라 체결가 기준이라 반응이 빠르다.

즉 `일봉 DOWN + 4h 이미 UP 반전 + 1h 애매` 조합이 숏으로 들어가던 경로가 막혔다.
모든 타임프레임이 같은 방향이면 그대로 통과한다 (테스트로 양쪽 다 검증).
...
- **진입 맥락** — 그 코인을 왜 골랐는지 (ADX·일관성·점수·ATR·현재변동성·24h등락)

리포트의 **진입 지표별 성적** 표가 "어떤 조건의 코인이 실제로 이겼나" 를 보여준다.
각 지표를 중앙값으로 갈라 위/아래 성적을 비교한다 — 높은쪽이 뚜렷하게 좋으면
그 지표의 하한을 올릴 근거가 되고, 차이가 없으면 그 지표는 선정에 쓸모가 없다.
